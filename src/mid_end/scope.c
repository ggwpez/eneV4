...
{
    assert(obj);
    assert(context_vector_push_back(obj->contexts, context_new()));        // add new context
}

void scope_leave(scope_t* obj)
{
    assert(obj);
    assert(context_vector_pop_back(obj->contexts));
}
...
int scope_transform_var_decls(scope_t* obj, var_decl_vector_t* nodes)
{
    assert(obj);
    assert(nodes);
...
context_add_t scope_add_vars(scope_t* obj, var_decl_vector_t* nodes)
{
    ...
    for (size_t i = 0; i < s; ++i)
    {
        ...
        CHECK_RET(scope_add_var(obj, var));
    }
...
context_add_t scope_add_fun(scope_t* obj, fun_decl_node_t* elem)
{
    ...
    scope_transform_var_decls(obj, elem->args);
